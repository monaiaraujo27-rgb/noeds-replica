      ...
      if(node.nodeType===3){
        ...
        if(t.indexOf("[")>=0){
          node.nodeValue=t.replace(/\[([^\]\[]{2,60})\]/g,function(m,key){
            ...
            // dígito nem símbolo de moeda. Isso apaga "[Nome]", "[endereço]",
            // "[confirmar convênios]" (placeholders) mas PRESERVA conteúdo
            // legítimo entre colchetes como "[R$ 30]", "[1]", "[2024]" - que
            // NÃO devem sumir do dossiê do cliente. O que não casar fica como
            // está (colchete visível é menos grave que apagar um valor real).
            if(/^[A-Za-zÀ-ÿ_][^0-9R$]*$/.test(kk)) return "";
            return m;
