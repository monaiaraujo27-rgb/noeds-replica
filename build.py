...
@media (max-width:640px){
  ...
  .nd-ideas, .nd-persona .p-cols { grid-template-columns:1fr; }
  .nd-timeline { flex-direction:column; gap:16px; }
  .nd-timeline .t-step::before { display:none; }
}
@media print {
  .nd-copy { display:none !important; }
  .nd-kpi, .nd-persona, .nd-idea, .nd-bubble, .nd-obj .o-cliente { background:#fff !important; color:#000 !important; border-color:#ccc !important; }
...
(async function(){
  ...
  // grade de KPIs; valor vazio/"Não informado" vira chip "A confirmar"
  function statTiles(items){
    ...
    (items||[]).forEach(function(it){
      ...
      if(!v || /^(n[aã]o informado|ponto a confirmar|a confirmar|-)$/i.test(v)) t.appendChild(el("span","nd-chip","A confirmar"));
  ...
  var R={
    ...
    bcg:function(d){
      ...
      fillSection("portfolio",function(f){
        ...
        wrap.innerHTML='<div class="noeds-axis" style="margin-bottom:.6rem">Crescimento de mercado ↑ · Participação →</div>'
          +'<div class="noeds-quad">'
          +q("Estrela",d.estrela,"Foco do investimento",pct.estrela)
    ...
    certificado:function(d){
      fillSection("resumo",function(f){
        ...
        var dt=new Date();
        selo.appendChild(el("p","s-e",("0"+dt.getDate()).slice(-2)+" · "+("0"+(dt.getMonth()+1)).slice(-2)+" · "+dt.getFullYear()));
        f.appendChild(selo);
      });
